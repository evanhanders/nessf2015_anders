...
\abstract{Dynamo action
	...
	the solar photosphere will be created and post-processed to obtain
	synthetic observables.  Such mock-observables will be 
	directly compared to data gathered by the Solar Dynamics 
...
days.  Through probing the Sun's
...
of spatial and temporal timescales.  Regardless, the
...
accounting for magnetic induction (\citealt{Charbonneau2014}).  Most current
simulations
...
have largely informed solar dynamo theory and generated 
self-sustaining, buoyantly-rising toroidal wreaths of magnetism with polarity
shifts on long time scales,
such as those shown in Fig. \ref{wreaths}\emph{b\&c}.  
...
in recent simulations of Sun-like stars.  There is disagreement
on whether the tachocline is a necessary ingredient
in sustaining the solar magnetic dynamo.  Cyclic, Sun-like dynamos
...
convective flow velocities than the Sun.  It has recently been discovered
that these simulations may not accurately capture the physics at work
in the Sun.  The self-consistently achieved amplitudes of 
large scale convective
motions in such simulations are much larger than those actually present in 
the Sun
...
which test the tachocline's role in generating the solar magnetic 
...
such simulations can provide greater certainty regarding the importance
of the tachocline in generating and maintaining the solar dynamo.
...
advisor, Ben Brown, to create my simulations.  The ASH code is the
...
(such as ASH) created by others for my own purposes. I also learned
...
which spans the entirety of the solar convective zone and reaches out to
the solar photosphere, displaying the behavior of the magnetic fields
at the observable surface of the Sun.  In efforts to accurately portray the
true physics at work, these simulations will capture supergranular 
scales.  
...
First, the small scale motion of granulation are roughly two orders of magnitude
...
would increase the number of CPU hours required for a project of this magnitude
...
theory of the dynamo is working properly.  
...
The proposed work fits perfectly with NASA's 2014 Strategic Plan objective
1.4:
...
observables which utilize an untested assumption as a theoretical cornerstone.
After recent simulations have disagreed regading the 
...
observables.  Only once these two sources of insight are brought together
will NASA's Science Plan goal of predicting extreme conditions in space
be realizable.  With the wealth of Solar Dynamics
Observatory data available and the upcoming launch of the Solar Orbiter, 
now is the time to see how accurately solar theory is reflected in observations
of the Sun.
